<template>
  <section class="container">
    ...
    <b-form @submit="searchBooks" class="mb-5">
      <b-input-group>
    ...
    <ul class="list-unstyled">
      <b-media no-body tag="li" v-for="(item, index) in items" class="mb-5">
        <b-media-aside class="align-items-start">
          <b-img width="110" slot="aside" :src="getItemImage(item)" alt="" />
        </b-media-aside>
        <b-media-body class="ml-3">
          <h4 class="mt-0">{{ item.volumeInfo.title }}</h4>
          <p>{{ getDescription(item.volumeInfo.description) }}</p>
          <p>著者：<template v-for="author in item.volumeInfo.authors">{{ author }} / </template> 発売日：{{ item.volumeInfo.publishedDate }}</p>
          <b-button variant="primary" @click="addBook(item)">本棚に追加</b-button>
          <b-button :href="`https://books.google.co.jp/books?id=${item.id}`" target="_blank" variant="success">詳細を見る</b-button>
        </b-media-body>
      </b-media>
    </ul>
...
export default {
  ...
  methods: {
    ...
    addBook(item) {
      ...
      if (item.volumeInfo.imageLinks) {
        const images = item.volumeInfo.imageLinks
        book.imageUrl = (images.thumbnail) ? images.thumbnail : images.smallThumbnail
      } else {
        book.imageUrl = ''
      }
