    ...
    <ul class="list-unstyled">
      <b-media no-body tag="li" v-for="(item, index) in items" class="mb-5">
        <b-media-aside class="align-items-start">
          <b-img width="110" slot="aside" :src="item.imageUrl" alt="" />
        </b-media-aside>
        <b-media-body class="ml-3">
          <h4 class="mt-0">{{ item.title }}</h4>
          <p>{{ getDescription(item.description) }}</p>
          <p>著者：<template v-for="author in item.authors">{{ author }} / </template> 発売日：{{ item.publishedDate }}</p>
          <b-button :href="`https://books.google.co.jp/books?id=${item.googleBooksId}`" target="_blank" variant="success">詳細を見る</b-button>
          <b-button variant="danger" @click="deleteBook(item, index)">削除</b-button>
        </b-media-body>
      </b-media>
    </ul>
  </section>
